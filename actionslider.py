...
class ActionSliderException(Exception):
    '''Raised when add_widget or remove_widget called incorrectly on an ActionSlider.

    '''


class ActionSlider(StencilView):
    ...
    # Defaults (see kv) to minimum of 300dp or half actionslider width
    ...
    def add_widget(self, widget):
        if len(self.children) == 0:
            super(ActionSlider, self).add_widget(widget)
            ...
            raise ActionSliderException(
                'Can\'t add widgets directly to ActionSlider')
...
if __name__ == '__main__':
    ...
    actionslider.add_widget(main_panel)
    main_panel.bind(size=main_panel.setter('text_size'))
    
    Window.add_widget(actionslider)
